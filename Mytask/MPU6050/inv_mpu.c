...
static int set_int_enable(unsigned char enable)
{
    ...
    if (st.chip_cfg.dmp_on) {
        if (enable)
            tmp = BIT_DMP_INT_EN;   //0x02
        else
            tmp = 0x00;
        if (i2c_write(st.hw->mpu_addr, st.reg->int_enable, 1, &tmp))
            return -1;
...
int mpu_reg_dump(void)
{
    ...
    for (ii = 0; ii < st.hw->num_reg; ii++) {
        if (ii == st.reg->fifo_r_w || ii == st.reg->mem_r_w)
            continue;
        if (i2c_read(st.hw->mpu_addr, ii, 1, &data))
            return -1;
        //log_i("%#5x: %#5x\r\n", ii, data);
    }
    return 0;
...
int mpu_read_reg(unsigned char reg, unsigned char *data)
{
    ...
    if (reg >= st.hw->num_reg)
        return -1;
    return i2c_read(st.hw->mpu_addr, reg, 1, data);
}
...
int mpu_init(void)
{
    ...
    if (i2c_write(st.hw->mpu_addr, st.reg->pwr_mgmt_1, 1, &(data[0])))
//        return -1;
    delay_vtask(100);
    ...
    if (i2c_read(st.hw->mpu_addr, st.reg->accel_offs, 6, data))
        return -1;
    ...
    if (rev) {
        ...
        if (rev == 1)
            ...
            Trace_log("Unsupported software product rev %d.\r\n");
            return -1;
        }
    } else {
        if (i2c_read(st.hw->mpu_addr, st.reg->prod_id, 1, &(data[0])))
            return -1;
        rev = data[0] & 0x0F;
        if (!rev) {
            Trace_log("Product ID read as 0 indicates device is either incompatible or an MPU3050.\r\n");
            return -1;
        } else if (rev == 4) {
            Trace_log("Half sensitivity part found.\r\n");
            st.chip_cfg.accel_half = 1;
        } else
            st.chip_cfg.accel_half = 0;
    }
...
int mpu_lp_accel_mode(unsigned char rate)
{
    ...
    if (!rate) {
        ...
        tmp[0] = 0;
        tmp[1] = BIT_STBY_XYZG;
        if (i2c_write(st.hw->mpu_addr, st.reg->pwr_mgmt_1, 2, tmp))
            return -1;
        st.chip_cfg.lp_accel_mode = 0;
        return 0;
    }
...
int mpu_get_gyro_reg(short *data, unsigned long *timestamp)
{
    ...
    if (i2c_read(st.hw->mpu_addr, st.reg->raw_gyro, 6, tmp))
        return -1;
...
int mpu_get_accel_reg(short *data, unsigned long *timestamp)
{
    ...
    if (i2c_read(st.hw->mpu_addr, st.reg->raw_accel, 6, tmp))
        return -1;
    data[0] = (tmp[0] << 8) | tmp[1];
    data[1] = (tmp[2] << 8) | tmp[3];
    data[2] = (tmp[4] << 8) | tmp[5];
...
int mpu_get_temperature(long *data, unsigned long *timestamp)
{
    ...
    if (i2c_read(st.hw->mpu_addr, st.reg->temp, 2, tmp))
        return -1;
    raw = (tmp[0] << 8) | tmp[1];
    if (timestamp)
        get_ms(timestamp);
...
int mpu_set_accel_bias(const long *accel_bias)
{
    ...
    if (i2c_read(st.hw->mpu_addr, 3, 3, data))
        return -1;
    fg[0] = ((data[0] >> 4) + 8) & 0xf;
    fg[1] = ((data[1] >> 4) + 8) & 0xf;
    fg[2] = ((data[2] >> 4) + 8) & 0xf;
    ...
    if (i2c_read(st.hw->mpu_addr, 0x06, 6, data))
        return -1;
    ...
    if (i2c_write(st.hw->mpu_addr, 0x06, 6, data))
        return -1;
    return 0;
}
...
int mpu_reset_fifo(void)
{
    ...
    data = 0;
    if (i2c_write(st.hw->mpu_addr, st.reg->int_enable, 1, &data))
        return -1;
    if (i2c_write(st.hw->mpu_addr, st.reg->fifo_en, 1, &data))
        return -1;
    if (i2c_write(st.hw->mpu_addr, st.reg->user_ctrl, 1, &data))
        return -1;
    ...
    if (st.chip_cfg.dmp_on) {
        ...
        if (i2c_write(st.hw->mpu_addr, st.reg->int_enable, 1, &data))
            return -1;
        if (i2c_write(st.hw->mpu_addr, st.reg->fifo_en, 1, &st.chip_cfg.fifo_enable))
            return -1;
    }
    return 0;
}
...
int mpu_set_gyro_fsr(unsigned short fsr)
{
    ...
    if (st.chip_cfg.gyro_fsr == (data >> 3))
        return 0;
    if (i2c_write(st.hw->mpu_addr, st.reg->gyro_cfg, 1, &data))
        return -1;
...
int mpu_set_accel_fsr(unsigned char fsr)
{
    ...
    if (st.chip_cfg.accel_fsr == (data >> 3))
        return 0;
    if (i2c_write(st.hw->mpu_addr, st.reg->accel_cfg, 1, &data))
        return -1;
...
int mpu_set_lpf(unsigned short lpf)
{
    ...
    if (st.chip_cfg.lpf == data)
        return 0;
    if (i2c_write(st.hw->mpu_addr, st.reg->lpf, 1, &data))
        return -1;
...
int mpu_set_sample_rate(unsigned short rate)
{
    ...
    if (st.chip_cfg.dmp_on)
        ...
        data = 1000 / rate - 1;
        if (i2c_write(st.hw->mpu_addr, st.reg->rate_div, 1, &data))
            return -1;
...
int mpu_configure_fifo(unsigned char sensors)
{
    ...
    if (st.chip_cfg.dmp_on)
        ...
        if (sensors || st.chip_cfg.lp_accel_mode)
            set_int_enable(0);  //set_int_enable(1);¸Ä
        else
            set_int_enable(0);
        if (sensors) {
            if (mpu_reset_fifo()) {
...
int mpu_set_sensors(unsigned char sensors)
{
    ...
    if (sensors & INV_XYZ_GYRO)
        ...
        data = BIT_SLEEP;
    if (i2c_write(st.hw->mpu_addr, st.reg->pwr_mgmt_1, 1, &data)) {
    ...
    if (!(sensors & INV_XYZ_ACCEL))
        data |= BIT_STBY_XYZA;
    if (i2c_write(st.hw->mpu_addr, st.reg->pwr_mgmt_2, 1, &data)) {
        st.chip_cfg.sensors = 0;
        return -1;
    }
...
int mpu_get_int_status(short *status)
{
    ...
    if (!st.chip_cfg.sensors)
        return -1;
    if (i2c_read(st.hw->mpu_addr, st.reg->dmp_int_status, 2, tmp))
        return -1;
        ...
        unsigned char *sensors, unsigned char *more)
{
    ...
    if (i2c_read(st.hw->mpu_addr, st.reg->fifo_count_h, 2, data))
        return -1;
...
//    log_i("FIFO count: %hd\n", fifo_count);
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        ...
        if (i2c_read(st.hw->mpu_addr, st.reg->int_status, 1, data))
            return -1;
    ...
    if (i2c_read(st.hw->mpu_addr, st.reg->fifo_r_w, packet_size, data))
        return -1;
    ...
    unsigned char *more)
{
    ...
    if (i2c_read(st.hw->mpu_addr, st.reg->fifo_count_h, 2, tmp))
        return -1;
    ...
    if (fifo_count > (st.hw->max_fifo >> 1)) 
		{
        /* FIFO is 50% full, better check overflow bit. */
        if (i2c_read(st.hw->mpu_addr, st.reg->int_status, 1, tmp))
            return -1;
        if (tmp[0] & BIT_FIFO_OVERFLOW) 
				{
            mpu_reset_fifo();
					  i2c_read(st.hw->mpu_addr, st.reg->int_status, 1, tmp);
					  if (tmp[0] & BIT_FIFO_OVERFLOW) 
              return -2;
        }
    }

    if (i2c_read(st.hw->mpu_addr, st.reg->fifo_r_w, length, data))
        return -1;
...
int mpu_set_bypass(unsigned char bypass_on)
{
    ...
    if (bypass_on) {
        if (i2c_read(st.hw->mpu_addr, st.reg->user_ctrl, 1, &tmp))
            return -1;
        tmp &= ~BIT_AUX_IF_EN;
        if (i2c_write(st.hw->mpu_addr, st.reg->user_ctrl, 1, &tmp))
            return -1;
        delay_ms(3);
        tmp = BIT_BYPASS_EN;
        ...
        if (i2c_write(st.hw->mpu_addr, st.reg->int_pin_cfg, 1, &tmp))
            return -1;
    } else {
...
static int get_accel_prod_shift(float *st_shift)
{
    ...
    if (i2c_read(st.hw->mpu_addr, 0x0D, 4, tmp))
        return 0x07;
...
static int gyro_self_test(long *bias_regular, long *bias_st)
{
    ...
    if (i2c_read(st.hw->mpu_addr, 0x0D, 3, tmp))
        return 0x07;
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    data[0] = 0x01;
    data[1] = 0;
    if (i2c_write(st.hw->mpu_addr, st.reg->pwr_mgmt_1, 2, data))
        return -1;
    delay_ms(200);
    data[0] = 0;
    if (i2c_write(st.hw->mpu_addr, st.reg->int_enable, 1, data))
    ...
    if (i2c_write(st.hw->mpu_addr, st.reg->pwr_mgmt_1, 1, data))
        return -1;
    if (i2c_write(st.hw->mpu_addr, st.reg->i2c_mst, 1, data))
        return -1;
    if (i2c_write(st.hw->mpu_addr, st.reg->user_ctrl, 1, data))
        return -1;
    ...
    for (ii = 0; ii < packet_count; ii++) {
        ...
        if (i2c_read(st.hw->mpu_addr, st.reg->fifo_r_w, MAX_PACKET_LENGTH, data))
            return -1;
        ...
        unsigned char *data)
{
    ...
    if (i2c_write(st.hw->mpu_addr, st.reg->bank_sel, 2, tmp))
        return -1;
    if (i2c_write(st.hw->mpu_addr, st.reg->mem_r_w, length, data))
        return -1;
    return 0;
}
        ...
        unsigned char *data)
{
    ...
    if (i2c_write(st.hw->mpu_addr, st.reg->bank_sel, 2, tmp))
        return -1;
    if (i2c_read(st.hw->mpu_addr, st.reg->mem_r_w, length, data))
        return -1;
    return 0;
}
...
#define LOAD_CHUNK  (16)
    ...
    if (i2c_write(st.hw->mpu_addr, st.reg->prgm_start_h, 2, tmp))
        return -1;
...
int mpu_set_dmp_state(unsigned char enable)
{
    ...
    if (enable) {
        ...
        tmp = 0;
        i2c_write(st.hw->mpu_addr, 0x23, 1, &tmp);
        st.chip_cfg.dmp_on = 1;
    ...
    unsigned char lpa_freq)
{
    ...
    if (lpa_freq) {
        ...
        data[0] = thresh_hw;
        data[1] = time;
        if (i2c_write(st.hw->mpu_addr, st.reg->motion_thr, 2, data))
            goto lp_int_restore;
